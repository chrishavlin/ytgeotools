...
class Dataset():
    ...
    def __init__(self, data: dict, coords: dict):
        ...
        self.bbox = np.array([ [self.coords[dim].min(), self.coords[dim].max()] for dim in range(3)])
...
class GeoSpherical(Dataset):
    ...
    def __init__(self,
                 ...
                 max_radius: float = 6371.,
                 ):

        self.max_radius = max_radius
        coord_init = {'latitude': latitude, "longitude": longitude, "depth": depth}
        coords = {idim: {"values": coord_init[dim], "name": dim} for idim, dim in enumerate(coord_order)}
        self.max_radius = max_radius
    ...
    def cartesian_coords(self) -> tuple[ArrayLike]:
        ...
        cmesh = np.meshgrid(self.coords[0], self.coords[1], self.coords[2], indexing="ij")
    ...
    def interpolate_to_uniform_cartesian(self, field_subset:list=None, N: int =500):
        ...
        xyz_int = [np.linspace(cart_bbox[0,d], cart_bbox[1,d], Ngrid[d]) for d in range(3)]
...
class XarrayGeoSpherical(GeoSpherical):
    ...
    def __init__(self,
                 ...
                 max_radius: Type[unyt_quantity] = unyt_quantity(6371., 'km')):
        ...
        with xr.open_dataset(filename) as ds:
            ...
            for f in field_subset:
                if "_".join(list(getattr(ds, f).coords)) != co_hash:
                    raise ValueError(f"all fields must have the same coordinate ordering but {f} does not."
        ...
        super().__init__(data, latitude, longitude, depth, coord_order=coord_order, max_radius=max_radius)
...
def load_uniform_grid(ds: Type[Dataset], *args, **kwargs):
    ...
    return lug(data, sizes, 1., *args, bbox=ds.bbox, geometry=geometry, **kwargs)
